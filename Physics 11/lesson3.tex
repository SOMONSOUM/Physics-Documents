...
\section{លំហាត់អនុវត្តន៍}
	...
	\end{enumerate}  
		...
		\draw [line width=2pt] (-.6,0) -- (.6,0);
		\end{tikzpicture}
		\caption{រូបសម្រាប់គណនាលំហាត់ទី ២៥ ២៦ និង​ ២៧}
	\end{figure}
	\item ដូចបង្ហាញក្នុងរូប គេមានចំណុចបន្ទុកអគ្គិសនីទី១ $3.94\mu C$ ត្រូវបានដាក់ត្រង់ $x_{1}=-4.7m$ និងចំណុចបន្ទុកអគ្គិសនីទី២ $6.14\mu C$ ត្រូវបានដាក់ត្រង់ $x_{2}=12.2m$។ តើតម្លៃនៃអាប់សុីស $x$ ដែលធ្វើឲ្យកម្លាំងផ្គួបរវាងបន្ទុកអគ្គិសនីទាំងពីរ មានអំពើលើបន្ទុកអគ្គិសនី មួយ​ទៀត $0.300\mu C$ ស្មើសូន្យ?
	\begin{figure}[H]
		\centering
		\begin{tikzpicture}
			\begin{scope}
				\draw[->, -Stealth] (0,0) -- (5,0) node at (2.5,0) [below left] {$0$};
				\shade[ball color=green!40] (1,0) circle[radius=6pt];
				\shade[ball color=green!40] (4,0) circle[radius=6pt];
				\node at (2.5,0) {$|$};
				\node at (1,.5) {$q_{1}$};
				\node at (4,.5) {$q_{2}$};
				\node at (1,-.5) {$x_{1}$};
...
\begin{enumerate}[k]
	...
	\shade [ball color=red!60] (0,4) circle (8pt);
	\node at (0,4) {$+$};
	\coordinate[label=above:$A(q)$] (A) at (0,4.2);
	\coordinate[label=below:$\overrightarrow{F}$] (F) at (5.5,3.8);
	\coordinate[label=above:$B(q_{1})$] (B) at (4,4.2);
	\draw [dashed] (0.3,4) -- (4,4);
	\draw [->, -Latex, line width=2pt] (4.2,4) --(6,4);
	\shade [ball color=red!60] (4,4) circle (8pt);
	\node at (4,4) {$+$};
	...
	\begin{scope}
	\coordinate[label=below:$\overrightarrow{F}$] (F) at (5.5,3.8);
	\coordinate[label=below:$\overrightarrow{E}$] (E) at (7.5,3.8);
	\coordinate[label=above:$A(q>0)$] (A) at (4,4.2);
	\draw [dashed] (0.3,4) -- (4,4);
	\draw [->, -Latex, line width=2pt] (4.2,4) --(6,4);
	\draw [->, -Latex, line width=2pt] (4.2,4) --(8,4);
	\shade [ball color=red!60] (4,4) circle (8pt);
	\node at (4,4) {$+$};
	\draw [->, -Latex, line width=2pt] (4,2) --(0.3,2);
	\draw [dashed] (0.3,2) -- (4,2);
	\draw [->, -Latex, line width=2pt] (4,2) --(8,2);
	\shade [ball color=gray!60] (4,2) circle (8pt);
	\node at (4,2) {$-$};
	\coordinate[label=below:$\overrightarrow{F}$] (F) at (7.5,1.8);
	\coordinate[label=below:$\overrightarrow{E}$] (E) at (0.5,1.8);
	\coordinate[label=above:$A(q<0)$] (A) at (4,2.2);
	\end{scope}
	\end{tikzpicture}
	\caption{វិុចទ័រ​​កម្លាំងអគ្គិសនី និងវុិចទ័រដែនអគ្គិសនី}
	\end{figure}
	\begin{itemize}
	\item ចំណុចចាប់នៅត្រង់ចំណុចបន្ទុកអគ្គិសនី
	\item ទិសជាទិសនៃកម្លាំងអគ្គិសនី $\overrightarrow{F}$
	\item ទិសដៅដូចកម្លាំង $\overrightarrow{F}$ បើ $q>0$ ហើយផ្ទុយពីកម្លាំង $\overrightarrow{F}$ បើ $q<0$
	\item អាំងតង់សុីតេ $E=\frac{F}{\abs{q}}$ ដែល $F$ គិតជា $(N)$, $q$ គិតជា $(C)$ និង $E$ គិតជា $(N/C)$ ឬ $(V/m)$
	\end{itemize}
	\end{remark}
	\item \emph{\kml ទ្រឹស្តីបទសម្រាប់គណនាលំហាត់}
	\begin{itemize}
		\item បន្ទុកអគ្គិសនី $Q>0$ បង្គិតត្រង់ចំណុចនីមួយៗជំុវិញវានូវដែន $\overrightarrow{E}$ ចាកផ្ចិត។
		\item បន្ទុកអគ្គិសនី $Q<0$ បង្គិតត្រង់ចំណុចនីមួយៗជំុវិញវានូវដែន $\overrightarrow{E}$ ចូលផ្ចិត។
	\end{itemize}
\end{enumerate}

\subsection{ដែនអគ្គិសនីបង្កើតដោយចំណុចបន្ទុកអគ្គិសនីតែមួយ}
\begin{enumerate}[m]
	...
	\begin{multicols}{2}
		...
		\node at (0,4) {$+$};
		\coordinate[label=above:$A(Q)$] (A) at (0,4.2);
		\coordinate[label=below:$\overrightarrow{E}$] (E) at (5.5,3.8);
		\coordinate[label=below:$\overrightarrow{F}_{A/B}$] (F) at (7.5,3.8);
		\coordinate[label=above:$B(q)$] (B) at (4,4.2);
		\draw [dashed] (0.3,4) -- (4,4);
		\draw [->, -Latex, line width=2pt] (4.2,4) --(6,4);
		\draw [->, -Latex, line width=2pt] (4.2,4) --(8,4);
		\shade [ball color=red!60] (4,4) circle (8pt);
	...
	\item \emph{\kml ខ្សែដែនអគ្គិសនី}
		\begin{definition}
			\emph{\kml ខ្សែដែនអគ្គសនី} គឺជាខ្សែដែលប៉ះនឹងវុិចទ័រដែនអគ្គិសនី $\overrightarrow{E}$ ត្រង់ចំណុចនីមួយៗរបស់វា។
			\begin{figure}[H]
				\centering
				\begin{tikzpicture}
					\begin{scope}
						\draw[line width=2pt] (0,0) .. controls (1,2) .. (3,3);
						\draw [->, -Latex, line width=1pt] (1,1.75) --(1.8,2.8);
						\node at (1,1.75) {$\bullet$};
						\draw [->, -Latex, line width=2pt] (0,0) --(.5,1);
					\end{scope}
				\end{tikzpicture}
			\end{figure}
			\emph{\kml ប្រភេទខ្សែដែនអគ្គិសនី}
				\begin{figure}[H]
					\begin{subfigure}[t]{0.5\textwidth}
						\centering
						\begin{tikzpicture}[scale=.8]
							\foreach \t in {30,60, ...,360} %boucle
							\draw[thick, directed] (0pt,0pt) -- (\t:3cm);%trait tracé en coordonnées polaires
							\shade[ball color=red!40] circle (9pt) node {$+$};
						\end{tikzpicture}
						\subcaption{ខ្សែដែននៃបន្ទុកវិជ្ជមាន}
					\end{subfigure}
					\begin{subfigure}[t]{0.5\textwidth}
						\centering
						\begin{tikzpicture}[scale=.8]
							\foreach \t in {30,60, ...,360} %boucle
							\draw[thick, direct] (0pt,0pt) -- (\t:3cm);
							\shade[ball color=blue!40] circle (9pt) node {$-$};
						\end{tikzpicture}
						\subcaption{ខ្សែដែននៃបន្ទុកអវិជ្ជមាន}
					\end{subfigure}
					\begin{subfigure}[t]{0.5\textwidth}
						\centering
						\includegraphics[scale=0.3]{field_3}
						\subcaption{ខ្សែដែននៃបន្ទុកពីរដែលមានសញ្ញាផ្ទុយគ្នា}
					\end{subfigure}
					\begin{subfigure}[t]{0.5\textwidth}
						\centering
						\includegraphics[scale=0.3]{field_4}
						\subcaption{ខ្សែដែននៃបន្ទុកពីរដែលមានសញ្ញាដូចគ្នា}
					\end{subfigure}
				\end{figure}
		\end{definition}
\end{enumerate}
%\subsection{ដែនអគ្គិសនីឯកសណ្ឋាន}
%\begin{enumerate}
%	\item \emph{\kml ដែនអគ្គិសនីឯកសណ្ឋាន}
%	\begin{definition}
%		\emph{\kml ដែនអគ្គិសនីឯកសណ្ឋានៈ} ជាដែនអគ្គិសនីដែលវុិចទ័រដែននៅគ្រប់ចំណុចទាំងអស់រក្សាទិសដៅនិងតម្លៃដូចៗគ្នា។\\ ខ្សែដែនអគ្គិសនីឯកសណ្ឋានជាបន្ទាត់ស្របៗគ្នា ហើយឃ្លាតស្មើគ្នា។
%	\end{definition}
%	\item \emph{\kml ដែនអគ្គិសនីនៅចន្លោះលោហៈពីរស្របគ្នា}
%\end{enumerate}
\section{លំហាត់អនុវត្តន៍}
\begin{enumerate}[m]
	\begin{multicols}{2}
		\item គេយកចំណុចបន្ទុកអគ្គិសនី $q=+2\times10^{-9}C$ ទៅដាក់ត្រង់កំពូល $A$ នៃត្រីកោណសម័ង្ស $ABC$ ដែលមានជ្រុង $a=30cm$។ គណនាដែនអគ្គិសនីដែលកើតមានត្រង់ចំណុចកណ្តាល $M$ នៃបាត $BC$។
					\begin{figure}[H]
		\centering
		\begin{tikzpicture}[scale=.8]
		\coordinate [label=above:$A(q>0)$] (A) at (1.0cm,4cm);
		\coordinate [label=left:$B$] (B) at (-1.0cm,1.0cm);
		\coordinate [label=right:$C$] (C) at (3cm,1.0cm);
		\coordinate [label=below:$M$] (M) at (1cm,1.0cm);
		\draw (A) -- (M);
		\draw (B) -- node {$\bullet$} (C) -- node[right] {$a$} (A) -- node[left] {$a$} (B);
		\tkzMarkAngle[size=.5cm,color=cyan,mark=|](C,B,A)
		\tkzMarkAngle[size=.5cm,color=magenta,mark=|](A,C,B)
		\tkzMarkAngle[size=.5cm,color=red,mark=|](B,A,C)
		\shade[ball color=cyan!60] (P) circle (8pt);
		\node at (A) {$+$};
		\end{tikzpicture}
	\end{figure}
	\end{multicols}
	\item គេយកចំណុចបន្ទុកអគ្គិសនីពីរគឺ $q_{1}=-10nC$ និង $q_{2}=40nC$ ទៅដាក់ត្រង់ពីរចំណុច $A$ និង $B$ ដែលមានចម្ងាយ $5cm$ ពីគ្នា។
	\begin{enumerate}[k]
		\item គណនាដែនអគ្គិសនីត្រង់ចំណុច $M$ មួយស្ថិតនៅចម្ងាយ $3cm$ ពី $A$ និង $4cm$ ពី $B$។
		\item គណនាកម្លាំងអគ្គិសនីរងដោយបន្ទុក $q'=-10^{-9}C$ ដាក់ត្រង់ $M$
	\end{enumerate}
	\item ចំណុចបន្ទុកអគ្គិសនីពីរ $q_{A}=5nC$ និង $q_{B}=-2nC$ ស្ថិតនៅរៀងគ្នាត្រង់ $A$ និង $B$ ដែល $AB=10cm$។
	\begin{enumerate}[k]
